<template>
	<view class="page">
		...
		<view class="form-section">
			...
			<view class="images"><view class="image-item" v-for="(url,index) in imageUrls" :key="url"><image :src="url" mode="aspectFill" @tap="preview(index)"/><text @tap="removeImage(index)">×</text></view><view class="add-image" v-if="imageUrls.length<3" @tap="chooseImages"><text class="add-symbol">＋</text><text>添加截图</text></view></view>
		...
		<view class="history" v-if="history.length">
			...
			<view class="history-row" v-for="item in history" :key="item.id">
				<view class="history-head"><text>{{typeText(item.feedbackType)}}</text><text class="status-label" :class="'status-'+item.handleStatus">{{statusText(item.handleStatus)}}</text></view>
				<text class="history-content">{{item.content}}</text>
				<view class="reply" v-if="item.reply"><text class="reply-title">平台回复</text><text>{{item.reply}}</text></view>
