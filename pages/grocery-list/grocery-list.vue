		...
		<scroll-view class="grocery-scroll" scroll-y>
			...
			<view class="grocery-list" v-if="groceries.length">
				<view class="grocery-row" :class="{done:item.checked}" v-for="item in groceries" :key="item.key" @tap="toggle(item)">
					...
					<view class="grocery-spec" @tap.stop>
						<text class="spec-label">采购量</text>
						<input class="spec-input" :value="item.purchaseSpec" maxlength="24" placeholder="如 500克" placeholder-class="spec-placeholder" @input="changeSpec(item,$event)" @blur="saveGroceries" />
					</view>
				</view>
			</view>
...
export default {
	...
	methods:{
		...
		parseSavedGroceries(value){try{const rows=typeof value==='string'?JSON.parse(value):value;return(Array.isArray(rows)?rows:[]).reduce((map,item)=>{if(item&&item.name)map[this.normalizeKey(item.name)]=item;return map},{})}catch(e){return{}}},
		serializeGroceries(){return this.groceries.map(item=>({name:item.name,purchaseSpec:String(item.purchaseSpec||'').trim(),quantity:Number(item.quantity)||1,dishNames:item.dishNames.slice(0,20),checked:!!item.checked}))},
		persist(){const value={};this.groceries.forEach(i=>{value[i.key]={checked:!!i.checked,purchaseSpec:String(i.purchaseSpec||'')}});uni.setStorageSync('groceryProgress:'+this.orderId,value);this.scheduleSave()},
		scheduleSave(){if(this.saveTimer)clearTimeout(this.saveTimer);this.saveTimer=setTimeout(()=>this.saveGroceries(),450)},
		async saveGroceries(){if(!/^\d+$/.test(this.orderId)||!this.groceries.length)return false;if(this.saveTimer){clearTimeout(this.saveTimer);this.saveTimer=null}try{await apiSaveGroceryList(this.orderId,this.serializeGroceries());return true}catch(e){return false}},
		changeSpec(item,event){item.purchaseSpec=String(event.detail.value||'').replace(/[\r\n]/g,'').slice(0,24);this.persist()},
		...
		async copyList(){if(!this.groceries.length){uni.showToast({title:'暂无可复制的食材',icon:'none'});return}await this.saveGroceries();const lines=['【采购清单】'];if(this.orderNo)lines.push('订单：#'+this.orderNo);lines.push('','要买的食材：');this.groceries.forEach((item,index)=>lines.push(`${index+1}. ${item.name}${item.purchaseSpec?' · '+item.purchaseSpec:item.quantity>1?' ×'+item.quantity:''}（用于${item.dishNames.join('、')}）`));if(this.dishes.length)lines.push('','对应菜品：'+this.dishes.map(item=>item.name).join('、'));uni.setClipboardData({data:lines.join('\n'),success:()=>uni.showToast({title:'采购清单已复制',icon:'success'})})},
