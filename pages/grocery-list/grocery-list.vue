	...
	<view class="grocery-page">
		...
		<scroll-view class="grocery-scroll" scroll-y>
			<view class="summary-card">
				<text class="summary-title">{{ progress === 100 && groceries.length ? '食材已经买齐' : '按清单采购，回家就能开做' }}</text>
...
export default {
	data(){return {orderId:'',orderNo:'',loading:true,dishes:[],groceries:[]}},
	computed:{checkedCount(){return this.groceries.filter(i=>i.checked).length},progress(){return this.groceries.length?Math.round(this.checkedCount/this.groceries.length*100):0}},
	onLoad(options){this.orderId=String(options.orderId||'');this.load()},
	methods:{
