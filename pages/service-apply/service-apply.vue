<template>
	<view class="page">
		...
		<view v-else-if="application && application.auditStatus !== '2'" class="status-panel">
			...
			<view class="status-row"><text>申请身份</text><text class="status-value">{{ roleName }}</text></view>
			<view class="status-row"><text>服务区域</text><text class="status-value">{{ regionText }}</text></view>
			<view v-if="application.providerId" class="status-row"><text>人员编号</text><text class="status-value">#{{ application.providerId }}</text></view>
		</view>
