<template>
	<view class="page">
		...
		<view class="space-shell" v-else>
			...
			<view class="mode-tabs-wrap">
				<view class="mode-tabs">
					<view class="mode-tab" :class="{active:activeSection===tab.key}" v-for="tab in sections" :key="tab.key" @tap="switchSection(tab.key)">
						<text>{{tab.label}}</text><text class="mode-tab-dot" v-if="activeSection===tab.key"></text>
					</view>
				</view>
			</view>
		<scroll-view class="body" scroll-y v-if="activeSection==='home'">
			...
			<view class="section-row shared-title"><text>双方共同菜单</text><text @tap="goOrder">去一起点菜</text></view>
		...
		<view class="order-host" v-else>
			<couple-order-panel ref="orderPanel" :key="activeSection" :remote="activeSection==='feed'"/>
		</view>
		</view>
...
<style>
	...
	.page{overflow:hidden}.head{padding-right:170rpx}.rank{flex:none;white-space:nowrap}.space-shell{height:calc(100vh - 166rpx);min-height:0;display:flex;flex-direction:column}.space-shell>.love-card{flex:none;margin-bottom:8rpx}.mode-tabs-wrap{flex:none;height:106rpx;padding:10rpx 22rpx 12rpx;background:transparent;box-sizing:border-box}.mode-tabs{height:82rpx;padding:6rpx;display:flex;align-items:center;border:1rpx solid #e4e9e7;border-radius:42rpx 42rpx 26rpx 26rpx;background:#e9eeec;box-sizing:border-box;overflow:visible}.mode-tab{position:relative;flex:1;height:68rpx;display:flex;align-items:center;justify-content:center;border-radius:34rpx;color:#737d79;font-size:25rpx;font-weight:700;transition:transform .2s ease-out,color .2s ease-out,background-color .2s ease-out}.mode-tab.active{height:82rpx;transform:translateY(-9rpx);border:1rpx solid #dcece6;border-radius:38rpx 38rpx 19rpx 19rpx;background:#fbfefd;color:#1eaa82;font-size:28rpx;font-weight:900;box-shadow:0 10rpx 24rpx rgba(42,102,84,.11)}.mode-tab-dot{position:absolute;left:50%;bottom:8rpx;width:28rpx;height:5rpx;transform:translateX(-50%);border-radius:4rpx;background:#35cda4}.body,.order-host{flex:1;min-height:0;height:0}
</style>
